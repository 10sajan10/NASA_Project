"""WRF-SFIRE plugged into the engine via the ModelAdapter contract.
...
Usage:
    from models.wrf_sfire_v2 import WRFSFireAsteroidModel
    from engine import to_engine_registry, Pipeline, PipelineRunner

    model = WRFSFireAsteroidModel(
...
Test mode (no binaries built):
    ...
    fakes the SFIRE outputs. The producer's staging logic still runs, so
    the end-to-end data flow is validated. See tests/test_wrf_sfire_v2.py.
"""
...
class WRFSFireAsteroidModel(ModelAdapter):
