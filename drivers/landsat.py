...
class LandsatHistoryDriver(Driver):
    ...
    def fetch(self, cube: Cube,
        ...
        for k, item in enumerate(items, start=1):
            ...
            ndvi = np.where(valid, self._index(nir, red), np.nan)
            ndwi = np.where(valid, self._index(nir, sw1), np.nan)
            nbr = np.where(valid, self._index(nir, sw2), np.nan)
            if np.isfinite(ndvi).sum() == 0:
                continue
            ndvi_stack.append(ndvi.astype(np.float32))
            ndwi_stack.append(ndwi.astype(np.float32))
            nbr_stack.append(nbr.astype(np.float32))
            dt = item.datetime
            if dt is None:
                dt = datetime.fromisoformat(item.properties["datetime"])
            ts.append(dt.replace(tzinfo=None))
            print(f"      Landsat scene {k}/{len(items)}  cloud="
                  f"{item.properties.get('eo:cloud_cover', float('nan')):.1f}%  "
                  f"valid_pixels={int(np.isfinite(ndvi).sum())}")

        if not ndvi_stack:
            raise RuntimeError("Landsat scenes found, but no valid pixels remained")

        src = (f"Landsat C2 L2 Planetary Computer; "
               f"{min(ts).date()}..{max(ts).date()}; "
               f"target season {self.target_date.month:02d}-"
               f"{self.target_date.day:02d} +/- {self.day_window}d")
        cube.write_3d("landsat_ndvi_hist", ts, np.stack(ndvi_stack),
                      source=src, native_res_m=30.0, units="dimensionless",
                      producer=self.name,
                      description="Historical Landsat NDVI seasonal samples")
        cube.write_3d("landsat_ndwi_hist", ts, np.stack(ndwi_stack),
                      source=src, native_res_m=30.0, units="dimensionless",
                      producer=self.name,
                      description="Historical Landsat NDWI seasonal samples")
        cube.write_3d("landsat_nbr_hist", ts, np.stack(nbr_stack),
                      source=src, native_res_m=30.0, units="dimensionless",
                      producer=self.name,
                      description="Historical Landsat NBR seasonal samples")
